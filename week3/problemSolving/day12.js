...
*/
//We will use a stack to add the elements and then pop them if they are exists in the array
function dirReduc(arr) {
  ...
  const opposites = {
    ...
    EAST: "WEST",
    WEST: "EAST",
  }; //map to store the oppisites
  for (let i = 0; i < arr.length; i++) {
    if (stackedarr.length == 0) {
      stackedarr.push(arr[i]); //First element if the array is empty
    } else if (opposites[arr[i]] === stackedarr[stackedarr.length - 1])
      //compare and pop
      stackedarr.pop();
  }
  return stackedarr;
...
console.log(dirReduc(["NORTH", "SOUTH", "SOUTH", "EAST", "WEST", "NORTH"])); // [] //don't need to move at all
